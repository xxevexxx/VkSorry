...
>Читает указанные диалоги
### Отправка сообщений
- `{CPREFIX}` __лс__ `{USER}` `{ENTER}` `{DATA}` - Отправит сообщение в лс пользователю
>Отправляет заданное сообщение и вложения в личные сообщения
### Текст аудио
- `{CPREFIX}` __слова__ - Вернет текст аудиозаписи
>Вернет текст пересланного/закрепленного аудио
### Демотиватор
- `{CPREFIX}` __дем__ `{ENTER}` `{TEXT}` `{ENTER}` `{TEXT}` - Создаст демотиватор
>Создаст демотиватор с текстом и прикрепленным изображением
### Текст в голосовое
- `{CPREFIX}` __озвучь__ `{ENTER}` `{TEXT}` - Озвучит текст
>Озвучит пересланный/вписанный текст
### Стикеры
- `{CPREFIX}` __стики__ `{USER}` - Вернет стикеры пользователя
>Вернет названия/общую стоимость стикеров пользователя
* [⬆️Навигация](#Навигация)
